...
async def test_get_all_cargoes(client: AsyncClient, session: AsyncSession, test_data_cargo, test_data_truck):
    ...
    assert response.json()[0]['nearest_trucks'] == 0
    assert response.json()[1]['nearest_trucks'] == 1
...
async def test_get_cargo(client: AsyncClient, session: AsyncSession, test_data_cargo: Cargo, test_data_truck):
    ...
    assert response.content == b'{"detail":"Cargo model instance not found."}'
    ...
    assert response.status_code == status.HTTP_200_OK, f"Expected status code {status.HTTP_200_OK}, but got {response.status_code}."
    assert response.json()['pick_up_location'] == {'city': pick_up_location.city, 'state': pick_up_location.state, 'postcode': pick_up_location.postcode}
    assert response.json()['delivery_location'] == {'city': delivery_location.city, 'state': delivery_location.state, 'postcode': delivery_location.postcode}
    assert response.json()['trucks'] == [{'VIN': truck.VIN, 'distance': 2239.76}]
...
async def test_add_cargo(client: AsyncClient, session: AsyncSession):
    ...
    assert response.json()['detail'][0]['msg'] == 'zip_pick_up and zip_delivery must be different'
